...
func (o *GetGameStateURL) Build() (*url.URL, error) {
	...
	var _path = "/game/{game_id}/state"

	gameID := o.GameID
	if gameID != "" {
		_path = strings.Replace(_path, "{game_id}", gameID, -1)
	} else {
		return nil, errors.New("gameId is required on GetGameStateURL")
	}
...
func (o *GetGameStateURL) BuildFull(scheme, host string) (*url.URL, error) {
	if scheme == "" {
		return nil, errors.New("scheme is required for a full url on GetGameStateURL")
	}
	if host == "" {
		return nil, errors.New("host is required for a full url on GetGameStateURL")
	}
