...
func (o *GetGameDescriptionURL) Build() (*url.URL, error) {
	...
	var _path = "/game/{game_id}"

	gameID := o.GameID
	if gameID != "" {
		_path = strings.Replace(_path, "{game_id}", gameID, -1)
	} else {
		return nil, errors.New("gameId is required on GetGameDescriptionURL")
	}
...
func (o *GetGameDescriptionURL) BuildFull(scheme, host string) (*url.URL, error) {
	if scheme == "" {
		return nil, errors.New("scheme is required for a full url on GetGameDescriptionURL")
	}
	if host == "" {
		return nil, errors.New("host is required for a full url on GetGameDescriptionURL")
	}
