...
func (o *PauseGameURL) Build() (*url.URL, error) {
	...
	var _path = "/game/{game_id}/pause"

	gameID := o.GameID
	if gameID != "" {
		_path = strings.Replace(_path, "{game_id}", gameID, -1)
	} else {
		return nil, errors.New("gameId is required on PauseGameURL")
	}
...
func (o *PauseGameURL) BuildFull(scheme, host string) (*url.URL, error) {
	if scheme == "" {
		return nil, errors.New("scheme is required for a full url on PauseGameURL")
	}
	if host == "" {
		return nil, errors.New("host is required for a full url on PauseGameURL")
	}
